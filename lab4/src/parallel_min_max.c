...
int main(int argc, char **argv) {
    ...
    if (timeout > 0) {
        sleep(timeout);  // Ждем завершения процессов в течение таймаута

        // Отправляем сигнал SIGKILL всем дочерним процессам
        for (int i = 0; i < pnum; i++) {
            if (kill(child_pids[i], 0) == 0) {  // Если процесс всё ещё активен
                kill(child_pids[i], SIGKILL);
                printf("Process %d was killed due to timeout\n", child_pids[i]);
            }
        }
    }
